...
const convert2HLS = async (inputPath: string, outputDir: string) => {
  ...
  try {
    await Deno.mkdir(outputDir, { recursive: true }); // --- 1. PROBE: Find ONLY AAC Stereo ---
    ...
    const allAudio = JSON.parse(decoder.decode(probeOut)).streams || []; // Find the index of the first AAC stereo track
    const targetAudio = allAudio.find(
      (s: any) => s.codec_name === "aac" && s.channels === 2,
    );
    if (!targetAudio) {
      return {
        status: false,
        message:
          "No AAC Stereo track found. Approach requires pre-existing AAC stereo.",
      };
    }
    const audioIdx = targetAudio.index;
    console.log(`Found AAC Stereo at stream index ${audioIdx}.`); // --- 2. FAST SPLIT (Now mapping only the target audio) ---
    ...
    const splitCmd = new Deno.Command("ffmpeg", {
      args: [
        ...
        "0:v:0",
        "-map",
        `0:${audioIdx}`, // Only take one video, one audio
        "-f",
        "segment",
        "-segment_time",
        "600",
    ...
    chunks.sort(); // --- 3. PARALLEL TS CONVERSION ---
      ...
      try {
        const cmd = new Deno.Command("ffmpeg", {
          args: [
            ...
            `${outputDir}/${chunk}`,
            "-c",
            "copy", // Instant copy because we know audio is already AAC
            "-f",
            "mpegts",
    ...
    await Promise.all(tasks); // --- 4. STITCH & HLS (With fixed tags) ---
    ...
    const result = await finalCmd.output(); // Cleanup

    await Deno.remove(listPath);
    for (let i = 0; i < chunks.length; i++)
      await Deno.remove(`${outputDir}/part_${i}.ts`);

    return {
      status: result.code === 0,
      message: result.code === 0 ? "Success" : "Stitch failed",
    };
  } catch (error: any) {
